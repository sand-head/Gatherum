...
/// nobody spelled the way the page did. Same premise as analysis: this points at a model
/// *you* run, and with no endpoint set nothing is registered, nothing is embedded, and
/// search is the full-text search it has always been.</summary>
public class EmbeddingOptions
{
    ...
    /// model asked for embeddings gives poor ones.</summary>
    public string Endpoint { get; set; } = "";
    ...
    /// <summary>Width of the vectors the model returns. It has to be stated rather than
    /// discovered because it is the width of a database column: startup reconciles the
    /// column to this number, and changing it re-embeds everything from scratch.</summary>
    ...
    /// <summary>Longest passage handed to the model in one piece. The default suits the
    /// 512-token window most embedding models have; raise it for a model with room.</summary>
    ...
    /// least-unrelated pages instead of empty. The right number is a property of the
    /// model — raise it if searches feel too literal, lower it if they wander.</summary>
    public double MaxDistance { get; set; } = 0.55;
