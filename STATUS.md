...
- **Bookmarks** — a URL captured as a file node, on request and never on a schedule:
  one self-contained sanitized HTML snapshot (styles and images inlined under a byte
  purse, scripts and frames stripped, references absolutized, source URL and capture
  time stamped in a first-line comment), rendered sandboxed in the file view, findable
  by the page's words and by its address, with "capture again" recording a fresh fetch
  as a new version. Non-HTML URLs (a PDF) are kept as the document they serve. The
  source URL lives on `FileBody` and in `meta.json`, so it survives a reindex; the
  seam is `IPageArchiver` (`HttpPageArchiver` + `PageSnapshot` today, a browser-driving
  archiver someday). New: `POST /api/bookmarks`, `POST /api/bookmarks/{id}/capture`,
  MCP `bookmark_page`/`capture_bookmark`, and `text/html` served inline is now
  CSP-sandboxed.
...
- **Tests** — 218 passing: the Markdown dialect (infobox/figure/callout round trips,
